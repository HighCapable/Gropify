...
在这里你可以同时配置所有配置类型的相关功能，这里的配置会向下应用到 [构建脚本配置](#构建脚本配置)、[源代码配置](#源代码配置)、[Android 项目配置](#android-项目配置)、[JVM 项目配置](#jvm-项目配置)、[Kotlin 多平台项目配置](#kotlin-多平台项目配置) 中。
...
common {
    // 启用功能
    //
    // 你可以分别对 [buildscript]、[sourceCode]、[android]、[jvm]、[kmp] 进行设置
    isEnabled = true
...
这里的配置会向下应用到 [Android 项目配置](#android-项目配置)、[JVM 项目配置](#jvm-项目配置)、[Kotlin 多平台项目配置](#kotlin-多平台项目配置) 中，你可以在对应项目类型的配置方法块中继续复写它。
...
sourceCode {
    ...
    //
    // Android、JVM 项目默认为 "main"
    // Kotlin 多平台项目默认为 "commonMain"
    sourceSetName = "main"
    ...
    //
    // Java、Kotlin 或 Kotlin 多平台项目默认使用项目设置的 `project.group`
    //
...
这里的配置包括 `common`、`sourceCode` 中的配置，你可以对其进行复写。
